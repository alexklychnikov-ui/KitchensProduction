# Управление заказами, материалами и этапами производства


<!-- MOCKUPS:START -->
![Mockup](Docs/mockups/mockup-20260818-225259.png)
<!-- MOCKUPS:END -->
...
**Передача заказчику:** см. [`HANDOVER.md`](HANDOVER.md) — дамп БД, docker-compose, чеклист приёмки. Пользовательская инструкция в Word: `delivery/READMEUser.docx`.

## Возможности

### Клиент (Telegram)

- Пошаговая воронка: стиль → длина → планировка → бюджет/класс → фасады → столешница → фурнитура → ориентир ₽ → телефон → заявка.
...
tests/                   — локальные pytest (в клиентский ZIP не включаются)
```
